 ...
 * after the app window is resized on Screen C (split screen in and out),
 * every Pressable on Screen B stops working for real finger taps.
 *
 * This variant runs fully edge-to-edge (`edgeToEdgeEnabled=true`), so the
 * keyboard does not resize the window — the trigger is the split-screen
 * resize instead. See the repo README for the mechanism and the keyboard
 * variant for apps that have not adopted edge-to-edge.
 */
...
function ScreenA({navigation}: any) {
  return (
    <ScrollView contentContainerStyle={{padding: 16}}>
      ...
      <Text style={STEP}>
        This app reproduces a react-native-screens bug: after the app window
        is resized (split screen in and out) while on Screen C, everything on
        Screen B stops responding to finger taps. The app runs fully
        edge-to-edge; the keyboard is never involved.
      </Text>
...
function ScreenB({navigation}: any) {
  // Simulates a busy app: guarantees React commits land while the window is
  // resized on Screen C. Capped so the UI eventually idles.
...
function ScreenC() {
  return (
    <ScrollView contentContainerStyle={{flexGrow: 1, padding: 16}}>
      <Text style={STEP}>
        Step 4: Put this app in split screen: open recents, tap this app's
        icon above its card, choose "Split screen", then pick any second app.
      </Text>
      <Text style={STEP}>
        Step 5: Drag the split divider all the way to the screen edge so this
        app is fullscreen again, then go back to Screen B.
      </Text>
    </ScrollView>
  );
}
